...
- `python -m app.run --grid` - Process 3x3 grid card back images from `cards/unprocessed_bulk_back/` with single-pass Claude Vision extraction
...
This is a trading card digitization system that uses Claude Vision to extract structured data from scanned trading card images.

### Data Flow
1. **INPUT**: 3x3 Grid Backs in `cards/unprocessed_bulk_back/` - **ONLY RAW INPUT TO PIPELINE**
2. **PROCESSING**: Single-pass Claude Vision extraction from grid back cards (9 cards per image)
...
### Processing Approach
- **Single-Pass Extraction**: Each 3x3 grid is sent once to Claude Vision with a simple, clear prompt
...
- Uses Claude Vision API (claude-opus-4-6) for single-pass extraction
...
- `ANTHROPIC_API_KEY` environment variable required for Claude Vision API
- `ANTHROPIC_MODEL` (optional) - Model to use (default: "claude-opus-4-6")
