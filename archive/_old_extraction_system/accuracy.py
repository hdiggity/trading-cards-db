...
class CardValidator:
    ...
    def _validate_single_card(self, card: Dict[str, Any]) -> Dict[str, Any]:
        ...
        corrected["copyright_year"] = self._validate_year(
            corrected.get("copyright_year")
        )
        ...
        if (
            corrected.get("copyright_year") in (None, "", "unknown", "n/a")
            and inferred_year
        ):
            corrected["copyright_year"] = inferred_year
        # If brand unknown but can be inferred from normalized set, fill it
        if corrected.get("brand") in (None, "", "unknown", "n/a") and isinstance(
            set_value, str
        ):
    ...
    def _validate_card_set(
        self, card_set: Any, brand: Any, year_hint: Any, name_hint: Any
        ...
        inferred_year = (
            extract_year(year_text) or extract_year(set_str) or extract_year(name_str)
        )
        ...
        has_subset_kw = (
            any(kw in set_str for kw in subset_keywords) if set_str else False
        )
        ...
        if (
            brand_is_known
            and (not set_str or set_str in {"unknown", "n/a", "none"})
            and inferred_year
        ):
            normalized = f"{inferred_year} {title_map[brand_guess]}"
            return normalized, inferred_year
        ...
        if (
            set_str
            and brand_is_known
            and title_map[brand_guess].lower() in set_str
            and inferred_year
            and not re.search(r"\b(19[6-9]\d|20[0-4]\d)\b", set_str)
        ):
    ...
    def _validate_player_card(self, card: Dict[str, Any]) -> bool:
        ...
        if "rookie" in features and not any(
            h in name for h in ["rookie first", "rookie stars"]
        ):
            return True
    ...
    def apply_smart_defaults(
        self, cards: List[Dict[str, Any]], context: Dict[str, Any] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _apply_context_defaults(
        ...
        # Apply year defaults
        if (
            not enhanced.get("copyright_year")
            or enhanced.get("copyright_year") in ["unknown", "n/a"]
            or str(enhanced.get("copyright_year", "")).strip() == ""
        ):
        ...
        if (
            not enhanced.get("brand")
            or enhanced.get("brand") in ["unknown", "n/a"]
            or str(enhanced.get("brand", "")).strip() == ""
        ):
        ...
        if (
            not enhanced.get("sport")
            or enhanced.get("sport") in ["unknown", "n/a"]
            or str(enhanced.get("sport", "")).strip() == ""
        ):
        ...
        # Apply set defaults
        if (
            not enhanced.get("card_set")
            or enhanced.get("card_set") in ["unknown", "n/a"]
            or str(enhanced.get("card_set", "")).strip() == ""
        ):
        ...
        if (
            not enhanced.get("team")
            or enhanced.get("team") in ["unknown", "n/a"]
            or str(enhanced.get("team", "")).strip() == ""
        ):
            ...
            if year:
                try:
                    ...
                    default_team = self._get_era_appropriate_team_default(
                        sport, year_int
                    )
                    if default_team:
    ...
    def _enhance_with_back_knowledge(self, card: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if enhanced.get("name") in ["unknown", "unidentified", None, ""]:
            ...
            enhanced_name = self._apply_brand_layout_knowledge(
                enhanced, brand, year_int, sport
            )
            if enhanced_name:
                enhanced["name"] = enhanced_name
                print(
                    f"Enhanced name using {brand} card back knowledge: {enhanced_name}",
                    file=sys.stderr,
                )

        return enhanced

    def _apply_brand_layout_knowledge(
        self, card: Dict[str, Any], brand: str, year: int, sport: str
    ) -> Optional[str]:
        """Apply specific brand layout knowledge to enhance name
        identification."""
    ...
    def _enforce_title_naming_rules(self, card: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def looks_like_person(n: str) -> bool:
            ...
            if 2 <= len(parts) <= 4 and all(
                re.match(r"^[A-Za-z\-']+$", p) for p in parts
            ):
                return True
            return False
        ...
        def build_title() -> str:
            ...
            # Rookie variants
                ...
                "rookie first base" in text
                or "rookie first basemen" in text
                or "first basemen" in text
            ):
            ...
            if title:
                if (
                    team
                    and team.lower() not in ["unknown", "n/a", "none"]
                    and "leaders" in title.lower()
                ):
                    base.append(team)
                base.append(title)
...
class ConfidenceScorer:
    ...
    def score_extraction(self, cards: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for card in cards:
            ...
            scored_card["_overall_confidence"] = self._overall_confidence(
                confidence_scores
            )
...
def build_enhanced_prompt() -> str:
    ...
    base_prompt = (
        """You are an expert trading card analyst with decades of experience. Your goal is PERFECT ACCURACY.
...
def _build_learning_corrections(insights: Dict[str, Any]) -> str:
    ...
    if insights.get("year_corrections"):
        corrections.append(
            "- Copyright year errors detected - focus extra attention on finding true © symbol"
        )

    if insights.get("name_corrections"):
        na_count = sum(
            1 for c in insights["name_corrections"] if c.get("original") == "n/a"
        )
        if na_count >= 2:
            corrections.append(
                "- Name identification needs improvement - make stronger effort to identify players"
            )

    if insights.get("condition_corrections"):
        corrections.append(
            "- Condition assessment often too optimistic - be more conservative in grading"
        )

    if insights.get("features_patterns"):
        corrections.append(
            "- Feature detection needs improvement - look more carefully for rookie, autograph, jersey indicators"
        )
    ...
    for brand, issues in brand_issues.items():
        if len(issues) >= 2:
            corrections.append(
                f"- {brand} cards often have errors - pay extra attention to accuracy"
            )
