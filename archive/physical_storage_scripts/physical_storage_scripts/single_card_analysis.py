...
def detect_card_border(image):
    ...
    approx = cv2.approxPolyDP(
        card_contour, 0.05 * peri, True
    )  # More tolerant approximation
...
def four_point_transform(image, pts):
    ...
    warped = cv2.warpPerspective(
        image, M, (maxWidth + 2 * dst_margin, maxHeight + 2 * dst_margin)
    )

    border = max(6, int(0.003 * min(warped.shape[0], warped.shape[1])))
    warped = cv2.copyMakeBorder(
        warped, border, border, border, border, cv2.BORDER_REPLICATE
    )
    return warped
...
def cleanup_processed_heics(root_path: str) -> int:
    """Remove HEIC/HEIF files that already have a matching *_optimized.png next
    to them.
...
def mirror_output(src_path: str, mirror_dir: str | None) -> str | None:
    """Copy src_path into mirror_dir, returning destination path if
    successful."""
    if not mirror_dir:
        return None
...
def remove_source_file(
    source_path: str, generated_path: str, keep_originals: bool
) -> bool:
    """Remove the original image when allowed.
    ...
    try:
        if os.path.exists(generated_path) and os.path.samefile(
...
def _encode_image_for_llm(filepath: str) -> tuple[str, str]:
    ...
    HEIC), convert to PNG bytes, return (b64, mime).
    """
    try:
...
def _build_llm_prompt() -> str:
    """Detailed, self-contained prompt for extracting card data from an
    image."""
    return (
...
def analyze_with_llm(
    image_path: str, model: str = None, temperature: float = 0.1, max_tokens: int = 2000
) -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            "openai package not installed; install `openai` or run without --llm"
        ) from e
    ...
    try:
        ...
        def norm(card: dict) -> dict:
            ...
            if isinstance(out.get("features"), str):
                ...
                out["features"] = (
                    ",".join(sorted(set([t for t in feats if t]))) if feats else "none"
                )
        ...
        if isinstance(data, dict) and isinstance(data.get("cards"), list):
            data["cards"] = [
                norm(c) if isinstance(c, dict) else c for c in data["cards"]
            ]
            return data
    except Exception:
        pass
...
def undo_processing():
    ...
    log_file = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "single_processed.txt"
    )
    original_formats = {}
...
def main():
    ...
    if args.llm:
        print(
            "JSON files saved to the specified output directory (default ~/Downloads)"
        )
    else:
        print(
            "Optimized images saved to the specified output directory (default ~/Downloads)"
        )
